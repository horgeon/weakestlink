...
export default class Vote extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({numberToExclude: currentSeq.numberToExclude, players: currentSeq.players.splice(0)});
    }
    ...
    handleVote(votedPlayerId) {
        this.setState((state) => {
            let votes = state.votes.splice(0);
    ...
    onEvent(event) {
        ...
        switch(event.type) {
            case 'VOTE_PLAYER_MODIFIED':
                ...
                this.setState({players: event.game.currentSequence.players.splice(0)});
                return true;
            case 'VOTE_PLAYER_DONE':
                this.context.setContext({game: event.game});
                this.setState({players: event.game.currentSequence.players.splice(0), done: true});
                return true;
            case 'VOTE_PLAYER_REFUSED':
                this.setState((state) => {
                    let i, votes = state.votes.splice(0);
    ...
    render() {
        ...
        if(currentSeq.type === 'VOTE') {
            if(this.context.gameMaster || this.context.gameAssistant) {
                let votes = [];
                let players = this.state.players.splice(0).sort((a, b) => a.currentRoundScore > b.currentRoundScore);
                players.forEach(player => {
                    votes.push((
                        <Table.Row>
                            <Table.Cell>{player.name}</Table.Cell>
                            <Table.Cell>{player.currentRoundScore}</Table.Cell>
